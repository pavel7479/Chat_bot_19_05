...
STAGE_02_MODULES = [
    ...
    "tests.testsOfBestVar.test_stage_02_semantic_compatibility",
]


class Stage02LoggingResult(unittest.TextTestResult):
    ...
    def stopTestRun(self) -> None:
        ...
        self._log_path.write_text(
            "\n".join(
                [
                    "Stage 02 Test Run Log",
                    f"generated_at={datetime.now(timezone.utc).isoformat()}",
                    "modules=" + ", ".join(STAGE_02_MODULES),
                    "",
                    *summary,
                    *self._blocks,
                ]
...
class Stage02LoggingRunner(unittest.TextTestRunner):
    def __init__(self, *, log_path: Path, **kwargs) -> None:
        self._log_path = log_path
        super().__init__(**kwargs)

    def _makeResult(self):
        return Stage02LoggingResult(
            self.stream,
            self.descriptions,
            self.verbosity,
...
def build_suite() -> unittest.TestSuite:
    ...
    for module_name in STAGE_02_MODULES:
        suite.addTests(loader.loadTestsFromName(module_name))
    return suite


def main() -> int:
    log_path = Path(__file__).resolve().parent / "tests_log_OfBestVar_stage_02"
    runner = Stage02LoggingRunner(verbosity=2, log_path=log_path)
